...
public class JoinPollResponsesServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String test = (String)request.getParameter("pollID");
		System.out.println("id here is "+test);
		if (!ProPollServer.checkForResponse(Integer.parseInt((String)request.getParameter("pollID")),(String) request.getSession().getAttribute("loggedOn"))) {
		...
		for (int i=0; i<(Integer)request.getSession().getAttribute("numquestions"); i++) {
			...
			int num = Integer.parseInt((String)request.getParameter("question"+Integer.toString(i)));
			System.out.println("USer chose " + num);
			responsenum.add(num);
		}
		ProPollServer.updateResponse(Integer.parseInt((String)request.getParameter("pollID")), responsenum, (String)request.getSession().getAttribute("loggedOn"));
		}
